...
usb_sts_type msc_disk_read(uint8_t lun, uint64_t addr, uint8_t *read_buf, uint32_t len)
{
  ...
  {
    case 0: /* SD card */
      if(sd_block_read(read_buf, (long long)addr, (uint16_t)len) != SD_OK)
      {
        return USB_FAIL;
      }
      break;
...
usb_sts_type msc_disk_write(uint8_t lun, uint64_t addr, uint8_t *buf, uint32_t len)
{
  ...
  {
    case 0: /* SD card */
      if(sd_block_write(buf, (long long)addr, (uint16_t)len) != SD_OK)
      {
        return USB_FAIL;
      }
      break;
...
usb_sts_type msc_disk_capacity(uint8_t lun, uint32_t *blk_nbr, uint32_t *blk_size)
{
  ...
  switch(lun)
  {
    case 0: /* SD card */
      *blk_size = 512u;
      *blk_nbr  = (uint32_t)(sd_card_info.card_capacity / 512LL);
      break;
    default:
      return USB_FAIL;
  }
